...
<body class="text-gray-800">
    ...
    <section class="px-8 py-12">
        ...
        <div class="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide">
            @foreach($cat as $c)
                <a href="/category/{{ encrypt($c->id) }}" 
                   class="min-w-[250px] bg-white shadow-md rounded-2xl overflow-hidden transform hover:scale-105 transition duration-300 block">
                    <img src="{{ $c->image ?? 'https://via.placeholder.com/300x200?text=Category' }}" 
                         alt="{{ $c->category }}" 
                         class="w-full h-40 object-cover">
                    <div class="p-4">
                        <h4 class="font-semibold text-lg text-gray-800">{{ $c->category }}</h4>
                        <p class="text-sm text-gray-500 mt-1">Explore our {{ strtolower($c->category) }} collection</p>
